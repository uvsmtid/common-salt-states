...
'{{ pillar['system_hosts'][grains['id']]['primary_user']['windows_user_home_dir'] }}\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\sshd_start.cmd':
    file.managed:
        - source: salt://common/ssh/sshd_start.cmd
        - template: jinja
        - require:
            - file: '{{ cygwin_root_dir }}\home\{{ pillar['system_hosts'][grains['id']]['primary_user']['username'] }}\sshd\sshd_config'
            - sls: common.cygwin.package

# Provide SSH server configuration:
'{{ cygwin_root_dir }}\home\{{ pillar['system_hosts'][grains['id']]['primary_user']['username'] }}\sshd\sshd_config':
    file.managed:
        - source: salt://common/ssh/sshd_config.cygwin
        - template: jinja
        - makedirs: true
        - require:
            - file: '{{ cygwin_root_dir }}\home\{{ pillar['system_hosts'][grains['id']]['primary_user']['username'] }}'
            - sls: common.cygwin.package

# Make sure directory exists:
'{{ cygwin_root_dir }}\home\{{ pillar['system_hosts'][grains['id']]['primary_user']['username'] }}':
    file.exists:
        - require:
            - sls: common.cygwin.package
            - cmd: run_login_shell
