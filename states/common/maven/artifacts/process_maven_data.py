...
def verify_maven_coords(
    ...
    for maven_coord in [
        ...
        if left_artifact[maven_coord] != right_artifact[maven_coord]:
            ...
            msg = 'Artifact `' + artifact_key + '` has different ' + maven_coord + ' = `' + left_artifact[maven_coord] + '` in ' + left_artifact_src + ' ' + maven_coord + ' = `' + right_artifact[maven_coord] + '` in ' + right_artifact_src + ' '

            logging.error(msg)
            auto_verification_target['verification_result'] = False
            auto_verification_target['error_messages'] += [ msg ]
...
def detect_ignorable_pom_file(
    ...
    if pom_file_data['is_exception']:
        ...
        logging.error(msg)
        artifact_descriptor['verification_result'] = False
        artifact_descriptor['error_messages'] += [ msg ]
        return True
...
def load_pom_files_data(
    ...
    for repo_id in report_data['pom_files'].keys():
        ...
        for pom_rel_path in pom_files.keys():
            ...
            # Create `step_logs` dict, if still missing.
            if 'step_logs' not in pom_file_data:
                pom_file_data['step_logs'] = {
            ...
            if pom_file_data['is_exception']:
                ...
                pom_file_data['step_logs']['warning_messages'] += [ msg ]
                continue

            if not pom_file_data['is_tracked']:
                msg = 'Pom `' + pom_rel_path + '` from `' + repo_id + '` repo is not tracked: ' + pom_abs_path
                logging.error(msg)
                pom_file_data['step_logs']['verification_result'] = False
                pom_file_data['step_logs']['error_messages'] += [ msg ]
                continue
            ...
            for artifact_key in single_pom_dependencies.keys():
                ...
                for xpath_key in single_pom_dependencies[artifact_key].keys():
                    ...
                    if 'step_logs' not in pom_dependency:
                        pom_dependency['step_logs'] = {
                            'verification_result': True,
                            'error_messages': [],
                            'warning_messages': [],
                        }
...
def load_artifact_descriptors_data(
    ...
    for artifact_key in report_data['artifact_descriptors'].keys():
        ...
        # Create `step_logs` dict, if still missing.
        if 'step_logs' not in artifact_descriptor:
            artifact_descriptor['step_logs'] = {
                'verification_result': True,
                'error_messages': [],
                'warning_messages': [],
            }
...
def verify_referential_integrity_pom_file_to_artifact_descriptors(
    ...
    def populate_reference_data(
        ...
        if 'step_logs' not in dependency_data:
            dependency_data['step_logs'] = {
                'verification_result': True,
                'error_messages': [],
                'warning_messages': [],
            }
        ...
        if artifact_key not in report_data['artifact_descriptors']:
            ...
            logging.error(msg)
            dependency_data['step_logs']['verification_result'] = False
            dependency_data['step_logs']['error_messages'] += [ msg ]
            return
        ...
        for pom_rel_path in report_data['pom_files'][repo_id].keys():
            ...
            for artifact_key in pom_file_data['maven_dependency_list'].keys():
                ...
                if artifact_key not in pom_file_data['xml_referenced_dependencies']:
                    ...
                    #       dependency or error?
                    dependency_data['warning_messages'] += [ msg ]
                else:
                    # Verify Maven Coordinates with each
                    # XML entry corresponding to `artifact_key`.
                    overall_verification_result = True
                    for xpath_key in pom_file_data['xml_referenced_dependencies'][artifact_key].keys():
                        verification_result = verify_maven_coords(
                            artifact_key,
                            dependency_data,
                        ...
                        if not verification_result:
                            overall_verification_result = False
                    return overall_verification_result
    ...
    for artifact_key in report_data['artifact_descriptors'].keys():
        ...
        if artifact_descriptor['source_type'] in [
            ...
            if repo_id not in report_data['pom_files']:
                ...
                logging.error(msg)
                artifact_descriptor['step_logs']['verification_result'] = False
                artifact_descriptor['step_logs']['error_messages'] += [ msg ]
                continue
