...
wait_for_online_jenkins_master_{{ unique_suffix }}:
    cmd.run:
        ...
        - name: 'COUNT_LIMIT=100; for ITER in $(seq "${COUNT_LIMIT}") ; do wget http://{{ jenkins_master_hostname }}:{{ jenkins_http_port }}/jnlpJars/jenkins-cli.jar -O {{ get_posix_salt_content_temp_dir() }}/jenkins/jenkins-cli.jar && break ; echo retrying... ; sleep 1; done ; test "${ITER}" != "${COUNT_LIMIT}"'
        - env:
